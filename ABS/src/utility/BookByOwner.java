...
/**
 * 
	Class that will make a booking by the business owner
 *
 */
public class BookByOwner implements BookAppointment {

	public BookByOwner() {

	}
	//The method that will complete the booking process
	//It should receive a list of slots to book and service selected by the owner
	public void doBooking(String[] customerData, String service,
		...
		int row = 0;
		int rows = toBookList.size();
		String selected = "";
		for (row = 0; row < rows; row++) {//Loop through every item to be booked 
			try {
				selected = toBookList.get(row); //set selected item is the current item in the list to be booked
					String recs[];
					while (true) {
					...
					String modified = recs[0] + "," + recs[1] + ","
							+ recs[2] + "," + recs[3] + "," + recs[4];//Set the line to modify un-available ....recs[4] 
		...
		JOptionPane.showMessageDialog(null, "Successfully Booked By Owner");
